...
--           (subst r 0 to,)
--             . (cs <>)
--             . (uncurry (<>) . first (S.singleton . flip Equal level . (,from)))
--             <$> typeOf' mcxt cxt r
...
--       v <- gen
--       cs <-
--         (uncurry (<>) . first (S.singleton . flip Equal level . (Type,)) <$> typeOf' mcxt cxt from)
--           <|> mzero
...
--           cs
--             <> toCs
--             <> S.singleton (Equal (Type, toType) (level + 1))
--         )
...
--             )
--       let everyBranchReturnsSame = fromList $ foldMap (\(x, xs) -> flip Equal level . (x,) <$> xs) $ uncons toTypes
--       pure (head toTypes, foldl (<>) mempty cs' <> everyBranchReturnsSame)
--     Case x _ cases -> undefined
--   where
--     typeOf' = typeOf level gcxt

infer :: Members U.Effects r => M.Map Id Term -> Term -> Term -> Sem r (Term, Term, Subst, S.Set Constraint)
infer cxt t1 t2 = do
  (subst, flexflex) <- unify $ S.singleton $ TypeOf t1 t2
  pure (manySubst subst t1, manySubst subst t2, subst, flexflex)
